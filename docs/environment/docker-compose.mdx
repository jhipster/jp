...
`docker-compose -f src/main/docker/app.yml stop` を実行します。

Dockerを直接操作することも可能です。
...
`Dockerfile`よりも推奨される方法です。この方法では、アプリケーションを構成するすべてのコンテナのメモリ構成に対して、単一のコントロールポイントを持つことができます。
