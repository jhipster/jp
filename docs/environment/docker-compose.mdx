...
開発、テスト、CIスクリプトではデフォルトでDocker Composeが使用されます。これは[無効化](#disabling)できます。
...
- 開発が容易になります。複雑なマイクロサービスアーキテクチャを使用している場合でも、1つのコマンドで完全なインフラストラクチャを起動できます。
- Docker Swarmを使用しているユーザーであれば、同じDocker Compose設定を使用するため、本番環境に直接デプロイできます。

Docker Composeの優れた機能の1つは、`docker-compose scale`コマンドを使用してコンテナをスケールできることです。これは、[マイクロサービスアーキテクチャ](#3)でJHipsterを使用する場合に非常に便利です。

アプリケーションを生成するとき、JHipsterは、データベースなどのサードパーティサービスでアプリケーションを実行するのに役立ついくつかのDocker Compose設定を生成します。これらのファイルは、`src/main/docker/`フォルダ内にあります。
...
Docker for MacとDocker for Windowsをダウンロードするには、Dockerストアのアカウントを作成する必要があります。ここではこれを省略します。

:::tip

WindowsとMac OS Xでは、KitematicはDocker Toolboxで提供される使いやすいグラフィカルインターフェースであり、Dockerをより簡単に使用できます。
...
Keycloakを動作させるには、hostsファイルに次の行を追加する必要があります（Mac/Linuxの場合は`/etc/hosts`、Windowsの場合は`c:\Windows\System32\Drivers\etc\hosts`）。
...
`docker-compose scale test-app=4`で、"test"アプリケーションの4つのインスタンスを実行します。

### コンテナの停止

`docker-compose -f src/main/docker/app.yml stop`

Dockerを直接操作することもできます。
...
コンテナで実行されるアプリケーションのメモリ使用量を最適化するために、`Dockerfile`または`docker-compose.yml`でJavaのメモリパラメータを設定できます。
...
Dockerfileよりも推奨される方法です。こうすることで、アプリケーションを構成するすべてのコンテナのメモリ構成に対して、単一のコントロールポイントを持つことができます。
...
Dockerベースイメージによっては、`JAVA_OPTS`が動作しない場合があります。この場合は、代わりに`_JAVA_OPTIONS`を試してみてください。
...
アプリケーションの設定に応じて、`src/main/resources/application.yml`または`application-dev.yml`に以下の内容を追加します。
