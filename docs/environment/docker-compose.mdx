...
- Docker Swarmを使用しているユーザであれば、同じDocker Compose設定を使用するため、本番環境に直接デプロイできます。
...
Dockerfileよりも推奨される方法です。この方法では、アプリケーションを構成するすべてのコンテナのメモリ構成に対して、単一のコントロールポイントを持つことができます。
...
アプリケーションの設定に応じて、`src/main/resources/application.yml`または`application-dev.yml`に以下の設定を追加します。
