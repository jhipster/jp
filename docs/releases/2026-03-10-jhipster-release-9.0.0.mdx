---
title: リリース 9.0.0
slug: 2026年3月10日のJHipsterリリース9.0.0.html
...
- **GraalVM support**: spring-boot:graalvm ジェネレーターを追加
- **Build tools**: Node 22、Gradle 9.4.0、Maven 3.9.13 に更新

### :computer: フロントエンド

- **React 19**: React 19 にアップグレードし、reactstrap を react-bootstrap に置き換え
- **Angular 21**: 主な更新点：
  - デフォルトで Zoneless になり、パフォーマンスが向上 (zone.js は不要に)
  - Jest の代替として Vitest をサポート
  - signals および httpResource ベースのサービスへの移行を継続
- **Vue**: Bootstrap 5 のサポートのため、Bootstrap Vue Next に移行
- **ESLint 10**: Angular および Vue 向けにアップグレード

### :warning: 破壊的変更

- **Java 21 必須**: Java 17 のサポートを終了 (Java 25 をサポート)
- **Node 22 必須**: Node 20 のサポートを終了、ネイティブ TypeScript のサポートが必須に

### :paw_prints: JDL/内部/ブループリント

- **TypeScript による完全な再設計**: ジェネレーターのコードベースは TypeScript で完全に書き直され、型改善とモジュラーアーキテクチャが導入されました
...
エンティティを一つずつ更新することもできます。例えば、エンティティ名が _Foo_ の場合、エンティティサブジェネレーターを再度実行します：
...
- Twitterで[@jhipster](https://twitter.com/jhipster)にお問い合わせください
