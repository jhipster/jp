...
- **Java 21 required**: Java 17 のサポートを終了 (Java 25 をサポート)
- **Node 22 required**: Node 20 のサポートを終了、ネイティブ TypeScript サポートが必須に

### :paw_prints: JDL/内部/ブループリント

- **Complete TypeScript redesign**: ジェネレーターのコードベースが TypeScript で完全に書き直され、型改善とモジュラーアーキテクチャを実現
...
エンティティを一つずつ更新するには、エンティティサブジェネレーターを再度実行します。例えば、エンティティ名が _Foo_ の場合：
