...
- **Angular 21**: 主な更新点:
  ...
  - signals と httpResource ベースのサービスへの移行を継続
...
- **完全な TypeScript による再設計**: ジェネレーターのコードベースが TypeScript で完全に書き直され、型とモジュラーアーキテクチャが改善されました
...
既存のアプリケーションを自動でアップグレードするには、[JHipster アップグレードサブジェネレーター](/upgrading-an-application/) を使用します:
...
手動でアップグレードするには、まず JHipster のバージョンを次のようにアップグレードします:
...
エンティティサブジェネレーターを再度実行することで、エンティティを1つずつ更新することもできます。例えば、エンティティ名が _Foo_ の場合:
