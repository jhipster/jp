...
>>>>>>> be9dc09c024a35e21e9637afdb55a6d53991f8d2:docs/options/using-cache.mdx
...
Caches are configured in the `CacheConfiguration` class, and can also be tuned using the JHipster [common application properties](/common-application-properties/).
...
- It is the default option for gateways, as we expect them to scale, and as Hazelcast is used to distribute the [gateway rate-limiting information](/api-gateway/#rate-limiting)
- When used within a monolith, Hazelcast needs to have the [JHipster Registry](/jhipster-registry/) configured manually in order to scale
>>>>>>> be9dc09c024a35e21e9637afdb55a6d53991f8d2:docs/options/using-cache.mdx
...
- It is configured using JHipster [common application properties](/common-application-properties/), using the key `jhipster.cache.hazelcast.management-center`, in your `application-dev.yml` and `application-prod.yml` files. Please note that it is turned off by default.
- JHipster generates a Docker Compose configuration to run the Hazelcast Management Center. Please read our [Docker Compose documentation](/docker-compose/), and run the application using `docker-compose -f src/main/docker/hazelcast-management-center.yml up -d`.
...
=======
[Infinispan](https://infinispan.org/) is a highly performant caching solution that can work as an in-memory local cache as well as clustered cache. It offers support for multiple cache modes:
  - [Local](https://infinispan.org/docs/9.4.x/user_guide/user_guide.html#local_mode)
  - [Invalidation](https://infinispan.org/docs/9.4.x/user_guide/user_guide.html#invalidation_mode)
  - [Distributed](https://infinispan.org/docs/9.4.x/user_guide/user_guide.html#replicated_mode)
  - [Replicated](https://infinispan.org/docs/9.4.x/user_guide/user_guide.html#distribution_mode)
...
=======
JHipster uses the popular [Xmemcached](https://github.com/killme2008/xmemcached) Java client for Memcached, and configures its most important properties using the usual JHipster [common application properties](/common-application-properties/).
>>>>>>> be9dc09c024a35e21e9637afdb55a6d53991f8d2:docs/options/using-cache.mdx
