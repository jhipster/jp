...
<Tabs lazy>
<TabItem value="dashboard" label="Auth0 ダッシュボードで設定" default>
...
<TabItem value="cli" label="Auth0 CLI を使用して設定">

#### Auth0 CLI を使用して設定する

1. Auth0アプリケーションを作成し、`.auth0.env`を生成します
    ```shell
    auth0 quickstarts セットアップ --app --type 通常 --framework jhipster
    ```
...
3. ロールを作成します (JHipsterは`ROLE_ADMIN`と`ROLE_USER`を想定しています)
    ```shell
