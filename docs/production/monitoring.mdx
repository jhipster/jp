...
JHipster Registryには[こちらに独自のドキュメントページ](/jhipster-registry/)があります。
...
JHipsterはLogstashへのログの転送をサポートしていますが、JHipsterバージョン7では、ELKスタックDockerデプロイメントは提供されておらず、ダッシュボードを使用する準備ができていません。これは、現在は保守されていない [JHipsterコンソール](https://github.com/jhipster/jhipster-console) サブプロジェクトの一部でした。既存のユーザーには、別のELKソリューションに移行することをお勧めします。
...
[jhipster-metrics-page]: /images/jhipster_metrics_page.png "JHipster Metrics page"
[grafana-micrometer-dashboard]: /images/monitoring_grafana_micrometer.png "Grafana Micrometer Dashboard" 
