...
デフォルトでは、コンフィギュレーションサーバは開発モードで実行されます。つまり、YAMLプロパティファイルはファイルシステムから直接読み込まれ、変更時にホットリロードされます。本番環境では、[JHipsterレジストリ設定サーバー](/jhipster-registry)および[Consul config server](/consul)のマイクロサービスドキュメントで説明されているように、Gitリポジトリから設定をセットアップすることをお勧めします。
...
**ヒント**:アプリケーションでprometheusメトリクスを有効にして保護する方法の詳細については、[モニタリングのドキュメント](/monitoring/#configuring-metrics-forwarding)を参照してください。
