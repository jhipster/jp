...
*   [JDL Studio](https://start.jhipster.tech/jdl-studio/)：ドメイン固有の言語[JDL](/jdl/)を使用してエンティティとリレーションシップを作成するためのオンライン・ツールです。
...
<<<<<<< HEAD
これは、[Springサービスサブジェネレータ](/creating-a-spring-service/)を使用するのと同じロジックであるため、詳細については参照先のドキュメントを読むことをお勧めします。
=======
...
<<<<<<< HEAD
もっと複雑なビジネス的な振る舞いが必要な場合は、[service sub-generator](/creating-a-service/)を使用して、Springの`@Service`クラスを追加する必要があります。
=======
